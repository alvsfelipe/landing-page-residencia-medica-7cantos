...
export function Z1Landing({ properties }: { properties: Property[] }) {
  ...
  return <main className="z1">
    ...
    <section className="z1-simulator" id="como-funciona">
      ...
      <form onSubmit={runSimulator} onFocus={() => track("simulator_started")}>
        <label>Onde você fará residência ou trabalhará?<select name="local" required defaultValue=""><option value="" disabled>Selecione uma opção</option><option>Hospital São Paulo</option><option>UNIFESP</option><option>Outro hospital da região</option><option>Ainda não sei</option></select></label>
    ...
    <section className="z1-properties" id="imoveis">
      <div className="z1-section-head"><p className="z1-eyebrow">Seleção local</p><h2>Imóveis na Vila Clementino</h2><p>Unidades compactas aparecem primeiro. O custo estimado soma aluguel, condomínio e IPTU informados na base.</p></div>
      <div className="z1-property-grid" id="resultado-imoveis">
        {compatible.slice(0, showAll ? compatible.length : 6).map((item, index) => {
          ...
          return <article className="z1-property" key={item.id || index} onMouseEnter={() => track("property_viewed", { property_id: item.id })}>
            <div className="z1-property-image">{image ? <>
              {/* eslint-disable-next-line @next/next/no-img-element -- origem dinâmica fornecida pela base local */}
              <img src={image} alt={`Imóvel na Vila Clementino${item.endereco ? ` — ${item.endereco}` : ""}`} />
            </> : <span>Foto não disponível</span>}</div>
