...
/** Busca uma página de imóveis. Lança `SeteCantosApiError` em falha definitiva. */
export async function fetchImoveisPage(options: ClientOptions, query: PageQuery = {}): Promise<ImovelListResponse> {
  const { token, baseUrl = DEFAULT_BASE_URL, timeoutMs = 30_000, maxRetries = 3, fetchImpl = fetch } = options;
  if (!token) throw new Error("Token da API da 7Cantos ausente (SETE_CANTOS_API_TOKEN).");

  const url = `${baseUrl.replace(/\/$/, "")}/imoveis?${buildQuery(query)}`;
